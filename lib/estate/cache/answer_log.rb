  ...
  module Cache
    ...
    module AnswerLog
      ...
      def record(name, outcome:, duration_ms: nil, fingerprint: nil, error: nil)
        ...
        Estate::Cache.logger.warn("Estate::Cache::AnswerLog: #{e.class}: #{e.message}")
        nil
      end

      def all
        Estate::Cache.store.read(KEY) || {}
      rescue StandardError
        {}
      end

      def clear!
        Estate::Cache.store.delete(KEY)
      end
      ...
      def write(entries)
        ...
        Estate::Cache.store.write(KEY, entries, expires_in: TTL)
      end
    end
  end
end
