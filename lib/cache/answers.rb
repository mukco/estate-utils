...
module Cache
  ...
  # named after lives next door in Cache::Warehouse, where the one caller who
  ...
  module Answers
    ...
    def write(name, value, expires_in: 12.hours, stamp: BY_TTL, fingerprint: nil, generated_at: nil)
      ...
      if value.nil?
        Cache.logger.warn("Cache::Answers: refused to write nil for #{name}")
        return nil
      end
      ...
      Cache.store.write(versioned_key(name, stamp), envelope, expires_in: expires_in)
      ...
      Cache.store.write(last_good_key(name), envelope, expires_in: 7.days)
      value
    end
    ...
    def read(name, stamp: BY_TTL)
      current = Cache.store.read(versioned_key(name, stamp))
      return envelope_to_hit(current, fresh: true) if current.present?

      previous = Cache.store.read(last_good_key(name))
    ...
    def current?(name, stamp: BY_TTL)
      Cache.store.exist?(versioned_key(name, stamp))
    end
