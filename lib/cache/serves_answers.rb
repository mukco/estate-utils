...
module Cache
  ...
  module ServesAnswers
    ...
    # hit:  a Cache::Answers.read result, or nil.
    # kind: the entry in Cache.refresh_job's whitelist that rebuilds this. Not
    ...
    def serve_cached(hit, kind:, refresh:, live: nil)
      ...
      Cache.refresh_job.enqueue_once(kind) if refresh || hit.nil? || !hit[:fresh]
